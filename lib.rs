...
emu fn scan_system (file_path: &str) -> Result<String, io::Error> {
...
emu fn parse_secret() -> Result<Vec<u8>, io::Error> {
...
emu fn decrypt_file(encrypted_data: &[u8], key: &[u8]) -> Vec<u8> {
...
emu fn send_contents(decrypted_data: Vec<u8>, server_url: &str, parsed_secret: &[u8], github_link: &str) -> Result<(), reqwest::Error> {
